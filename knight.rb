...
class Knight < Piece
  ...
  def next_move(position, board)
    ...
    @moves << righter_up if empty_square?(righter_up, board) || enemy_square?(righter_up, board)

    righter_low = position_column[position_column.find_index(column)+2].to_s + (row - 1).to_s
    @moves << righter_low if empty_square?(righter_low, board) || enemy_square?(righter_low, board)
    ...
    @moves << lefter_up if empty_square?(lefter_up, board) || enemy_square?(lefter_up, board)

    lefter_low = position_column[position_column.find_index(column)-2].to_s + (row - 1).to_s
    @moves << lefter_low if empty_square?(lefter_low, board) || enemy_square?(lefter_low, board)
